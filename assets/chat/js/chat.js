...
class Chat {
    ...
    withGui() {
        ...
        document.addEventListener('visibilitychange', debounce(100, () => {
            this.ishidden = (document['visibilityState'] || 'visible') !== 'visible';
            if (!this.ishidden) { focusIfNothingSelected(this); } else { ChatMenu.closeMenus(this); }
        }), true);
    ...
    addMessage(message, win = null) {
        ...
        if (message.type === MessageTypes.USER && this.ignored(message.user.nick, message.message)) { return; }
        ...
        if (win.lastmessage && win.lastmessage.type === MessageTypes.EMOTE && win.lastmessage.emotecount > 1) { win.lastmessage.completeCombo(); }
    ...
    onCLOSE({retryMilli}) {
        ...
        if (retryMilli > 0) { MessageBuilder.error(`Disconnected, retry in ${Math.round(retryMilli / 1000)} seconds ...`).into(this); } else { MessageBuilder.error(`Disconnected.`).into(this); }
    }
    ...
    onPRIVMSG(data) {
        ...
        if (!this.ignored(normalized, data.data)) {
            if (!this.whispers.has(normalized)) { this.whispers.set(normalized, {nick: data.nick, unread: 0, open: false}); }
            ...
            if (this.settings.get('showhispersinchat')) { MessageBuilder.whisper(data.data, user, this.user.username, data.timestamp, messageid).into(this); }
            if (this.settings.get('notificationwhisper') && this.ishidden) { Chat.showNotification(`${data.nick} whispered ...`, data.data, data.timestamp, this.settings.get('notificationtimeout')); }

            const win = this.getWindow(normalized);
            if (win) { MessageBuilder.historical(data.data, user, data.timestamp).into(this, win); }
            if (win === this.getActiveWindow()) { $.ajax({url: `${API_URI}/api/messages/msg/${messageid}/open`, method: 'post'}); } else { conv.unread++; }
